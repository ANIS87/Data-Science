...
class matrix_transition(object):
  ...
  def get_all_states(self,id_user):
     ...
     self.States_event=self.elementary_behaviour['event']['States']
  ...
  def calculate_transition_probabilities(self, id_user):
      ...
      #product to event
      self.Matrix_product_to_event=training_librairie.transition_product_to_clikc( self.data,self.visited_product)
  ...
  def function_kill_0(self):
     for i in range(self.Transition_Matrix.shape[0]):
       a=self.Transition_Matrix[i,:]
       if (a.sum()==0):
  ...
  def get_transition_matrix(self,id_user ):
     ...
     self.filling_insertion(  self.Matrix_product_to_event,    self.visited_product,   self.States_event )
     self.function_kill_0()
